...
describe("data ingestion authorization and validation", () => {
  it("rejects source management for non-admin users", async () => { const caller = appRouter.createCaller(context("user")); await expect(caller.data.sources()).rejects.toMatchObject({ code: "FORBIDDEN" }); await expect(caller.data.registerSource({ name: "CRM", type: "api", connectionRef: "https://internal.example/api" })).rejects.toMatchObject({ code: "FORBIDDEN" }); });
  it("rejects unsupported manual upload extensions before storage", async () => { const caller = appRouter.createCaller(context("admin")); await expect(caller.data.manualImport({ sourceName: "Network", fileName: "payload.exe", mimeType: "application/octet-stream", base64: "dGVzdA==" })).rejects.toMatchObject({ code: "BAD_REQUEST" }); });
  it("parses CSV rows and returns row-level validation details", async () => { const caller = appRouter.createCaller(context("admin")); const csv = Buffer.from("site_code,status\nAMM-001,healthy\nAMM-002,warning\nBROKEN\n").toString("base64"); const result = await caller.data.manualImport({ sourceName: "Network", fileName: "network.csv", mimeType: "text/csv", base64: csv }); expect(result.rowCount).toBe(3); expect(result.validRows).toBe(2); expect(result.invalidRows).toBe(1); expect(result.schema).toEqual(["site_code", "status"]); expect(result.errors).toEqual(expect.arrayContaining([expect.stringContaining("row 4")])); });
  ...
  it("records source configuration and mapping actions server-side", async () => { const caller = appRouter.createCaller(context("admin")); await expect(caller.data.registerSource({ name: "Network OSS", type: "sftp", connectionRef: "sftp://nms.internal/inbox" })).resolves.toEqual({ success: true }); await expect(caller.data.saveMapping({ importRunId: 1, mapping: { site_code: "siteCode" } })).resolves.toEqual({ success: true }); expect(dbSpy.created).toHaveLength(1); expect(dbSpy.mappings).toHaveLength(1); });
});
