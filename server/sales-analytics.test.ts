...
import { assembleSalesOperations, buildSalesOpportunity, createPreviewSalesOperations, isFiberReady, isNetworkReady, type SalesOpportunityInput } from "./sales-analytics";
...
describe("sales intelligence analytics", () => {
  ...
  it("builds the commercial preview with the Amman West network warning", () => {
    const result = createPreviewSalesOperations([{ id: "AMW-042", name: "Amman West", lat: 31.9539, lng: 35.9106, congestion: 94, fiber: 92, status: "healthy" }]);
    expect(result.opportunities[0]).toMatchObject({ value: 250000, networkIssue: true, alert: "Network issue may affect this opportunity." });
  });
});
...
describe("sales.operations procedure", () => {
  ...
  it("falls back to the operational sales preview", async () => { getPersistedSalesOperations.mockResolvedValueOnce(null); const result = await appRouter.createCaller(context()).sales.operations(); expect(result.source).toBe("operational-preview"); expect(result.summary.opportunities).toBeGreaterThan(0); });
});
