...
import { assembleBusinessRevenueOperations, buildBusinessRevenueArea, createPreviewBusinessRevenueOperations, type BusinessRevenueInput } from "./business-revenue-analytics";
...
describe("business and revenue analytics", () => {
  ...
  it("creates the requested 185K JOD/month preview exposure", () => {
    const result = createPreviewBusinessRevenueOperations([{ name: "Amman West", congestion: 94, customers: 8420, complaints: 128, churn: 3.8, revenueRisk: 286000, salesOpportunities: 14, fiber: 92 }]);
    expect(result.areas[0]).toMatchObject({ revenueAtRisk: 185000, networkIssue: true, action: "Fiber migration" });
  });
});
...
describe("businessRevenue.operations procedure", () => {
  ...
  it("falls back to operational preview", async () => { getPersistedBusinessRevenueOperations.mockResolvedValueOnce(null); const result = await appRouter.createCaller(context()).businessRevenue.operations(); expect(result.source).toBe("operational-preview"); expect(result.areas.length).toBeGreaterThan(0); });
});
